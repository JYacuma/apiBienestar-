...
class RegistroEstudianteFragment : Fragment(R.layout.fragment_registro_estudiante) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val etCodigo = view.findViewById<TextInputEditText>(R.id.etCodigoRegistro)
        val etPrograma = view.findViewById<TextInputEditText>(R.id.etProgramaRegistro)
        ...
        viewModel.registroResult.observe(viewLifecycleOwner) { result ->
            result.onSuccess {
                Toast.makeText(requireContext(), "¡Estudiante registrado con éxito!", Toast.LENGTH_SHORT).show()

                // 🎯 Esto cierra el RegistroActivity y vuelve al LoginActivity
        ...
        btnRegistrar.setOnClickListener {
            ...
            if (nombre.isEmpty() || correo.isEmpty() || contrasena.isEmpty() || codigo.isEmpty() || programa.isEmpty()) {
            ...
            val body = hashMapOf(
                ...
                "codigo" to codigo, "programa" to programa
            )
            viewModel.registrarEstudiante(body)
        }
    }
}